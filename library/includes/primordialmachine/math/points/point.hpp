...
struct is_point : public false_type
{}; // struct is_point

template<typename TYPE>
inline constexpr bool is_point_v = is_point<TYPE, void>::value;

template<typename T>
struct number_of_elements<T, enable_if_t<is_point_v<T>>>
{
  static constexpr size_t value = T::traits_type::number_of_elements;
}; // struct number_of_elements

template<typename T>
struct element_type<T, enable_if_t<is_point_v<T>>>
{
  using type = typename T::traits_type::element_type;
}; // struct element_type

template<typename T>
struct is_non_scalar<T, enable_if_t<is_point_v<T>>> : public true_type
{}; // struct is_non_scalar

template<typename T>
struct is_non_degenerate<T, enable_if_t<is_point_v<T>>>
{
  static constexpr bool value = T::traits_type::is_non_degenerate;
}; // struct is_non_degenerate

template<typename T>
struct is_degenerate<T, enable_if_t<is_point_v<T>>>
{
